...
% 下述代码用于优化 \smash。代码来自 https://tex.stackexchange.com/questions/759247 。
...
\def\RegistVarType@opt#1[#2]{
  \ExpandArgs{c}\def{var#1colorBG}{}
  \ExpandArgs{c}\def{var#1font}{}
  \pgfkeys{%
  ...
  \ExpandArgs{c}\def{var#1}##1{%
    \setlength{\fboxsep}{0pt}%
    \setlength{\fboxrule}{0pt}%
    \let\RegistVar@Cache=\relax  
    \ExpandArgs{c}\ifx{var#1colorBG}\empty
      \def\RegistVar@Cache{\fbox}%
    \else
      \def\RegistVar@Cache{\colorbox{\UseName{var#1colorBG}}}%
    \fi
    \RegistVar@Cache{\ensuremath{\plr{\UseName{var#1font}##1}}}%
  }
}
% ------ 常用变量注册模板 带高亮
...
\def\RegistVarFamily@#1#2[#3]{%
  \ExpandArgs{c}\def{var#2#1display}{#1}%
  \ExpandArgs{c}\def{var#2#1primes}{0}%
  \pgfkeys{
  ...
  \RegistVarFamilyLoop{#1}{#2}
    {\UseName{var#2#1display}}
    {\UseName{var#2#1primes}}
}
\ExplSyntaxOn
